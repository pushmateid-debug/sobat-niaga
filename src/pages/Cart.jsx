...
import { Trash2, Minus, Plus, ShoppingBag, TicketPercent, Loader2, Edit3 } from 'lucide-react';
...
const Cart = ({ onBack, user, onCheckout }) => {
  ...
  let totalPrice = cartItems.filter(item => item.selected).reduce((acc, item) => acc + (item.price * item.quantity), 0);
                ...
                <div className={`p-5 rounded-xl shadow-sm border sticky top-24 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'}`}>
                    ...
                    <div className="space-y-2 mb-4 text-sm">
                        ...
                        <div className={`flex justify-between ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                            ...
                            <span className={`${isDarkMode ? 'text-green-400' : 'text-green-600'}`}>-Rp 0</span>
